...
performance, so they are not run in production. They are just for development,
not needed in production. So they are removed by Babel plugins when a production
version is build.
